...
/*
attr_decl_list_second: 
	{[|[], []|]}
	| non_zero_attr_decl_list_second { $1 }
;

non_zero_attr_decl_list_second:
	attr_decl { [|[$1], []|] }
	| attr_decl non_zero_attr_decl_list_second  { [| ($2.(0)) , $2.(1) |]}
	| method_decl { [|[], [$1]|] }
;
*/

/* Make two seperate lists for holding attributes and the first method 
   We want to make this left recursive so the method can stop as soon as
   it encounters a method declaration, because we only want the first one. */

attr_decl_list:
	{ ([], []) } 
	| non_zero_attr_decl_list { $1 }
;
